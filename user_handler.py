...
class UserHandlers:
    ...
    # ══ جلب الروابط ═══════════════════════════════════════════════
    # ══ جلب الروابط — Flow الجديد ════════════════════════════════

    async def fetch_links_menu(self, update, ctx):
        """شاشة الدخول: إذا لا يوجد رقم → اختيار رقم أولاً."""
        uid  = update.effective_user.id
        nums = [n for n in self.db.get_user_numbers(uid) if n["is_active"]]

        if not nums:
            await self.safe_edit(update.callback_query,
                "📭 **لا توجد أرقام مفعّلة**\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "يجب إضافة رقم أولاً من قسم **إدارة الأرقام** قبل استخدام الجلب.",
                InlineKeyboardMarkup([
                    [btn("📱 إدارة الأرقام", "manage_numbers")],
                    [btn("🔙 رجوع", "main_menu")]
                ]))
            return
        ...
        # إذا لم يختر رقماً بعد → انتقل مباشرة لاختيار الرقم
        if not nid:
            await self.fetch_select_number(update, ctx)
            return

        # إذا عنده رقم → شاشة التأكيد
        await self._fetch_confirm_screen(update, ctx)

    async def _fetch_confirm_screen(self, update, ctx):
        """شاشة التأكيد النهائية قبل بدء الجلب."""
        uid  = update.effective_user.id
        nid  = ctx.user_data.get("fetch_nid")
        ltyp = ctx.user_data.get("fetch_type", "both")
        days = ctx.user_data.get("fetch_days", 7)

        dlbl  = {1: "آخر 24 ساعة", 7: "آخر أسبوع", 30: "آخر شهر", 90: "آخر 3 أشهر"}.get(days, f"{days} يوم")
        tlbl  = {"telegram": "📲 تيليجرام فقط", "whatsapp": "💬 واتساب فقط", "both": "📲+💬 الاثنين"}.get(ltyp, ltyp)

        n_obj = self.db.get_number(nid)
        n_lbl = n_obj["phone"] if n_obj else f"#{nid}"

        mem_count = self.fetch.temp_count
        mem_txt   = f"`{mem_count}` رابط محفوظ" if mem_count > 0 else "فارغة"

        is_running = self.fetch.is_running(uid)
        status_txt = "⚡ **جلب جارٍ الآن** — يمكنك إيقافه أو الانتظار." if is_running else ""

        text = (
            f"🔍 **جلب الروابط — جاهز للبدء**\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📱 الرقم: `{n_lbl}`\n"
            f"📡 النوع: {tlbl}\n"
            f"📅 النطاق: {dlbl}\n"
        ...
        kb_rows = [
            [btn("✏️ تغيير الرقم",  "fetch_select_number"),
             btn("✏️ تغيير النوع",  "fetch_select_type")],
            [btn("✏️ تغيير التاريخ","fetch_date_settings"),
             btn("🧹 تنظيف الذاكرة","fetch_clear_memory")],
        ]
        if is_running:
            kb_rows.append([btn("⏹️ إيقاف الجلب", "fetch_stop")])
        else:
            kb_rows.append([btn("🚀 بدء الجلب", "fetch_start")])
        kb_rows.append([btn("🔙 رجوع", "main_menu")])

        await self.safe_edit(update.callback_query, text, InlineKeyboardMarkup(kb_rows))

    async def fetch_select_number(self, update, ctx):
        """خطوة 1: اختيار الرقم — إذا رقم واحد يختاره تلقائياً."""
        uid  = update.effective_user.id
        nums = [n for n in self.db.get_user_numbers(uid) if n["is_active"]]
        ...
        # رقم واحد فقط → اختياره تلقائياً والانتقال للخطوة التالية
        if len(nums) == 1:
            ctx.user_data["fetch_nid"] = nums[0]["id"]
            await self.safe_ans(update.callback_query, f"✅ تم اختيار {nums[0]['phone']} تلقائياً")
            await self.fetch_select_type(update, ctx)
            return

        rows = [
            [btn(f"🟢 {n['phone']}", f"fetch_set_number_{n['id']}")]
            for n in nums
        ]
        rows.append([btn("🔙 رجوع", "main_menu")])
        await self.safe_edit(update.callback_query,
            "📱 **اختر الرقم:**\n━━━━━━━━━━━━━━━━━━━━━━",
    ...
    async def fetch_set_number(self, update, ctx, nid):
        ...
        # انتقل للخطوة التالية: اختيار النوع
        await self.fetch_select_type(update, ctx)

    async def fetch_select_type(self, update, ctx):
        """خطوة 2: اختيار نوع الروابط."""
        ltyp = ctx.user_data.get("fetch_type", "both")
        def mark(v): return "✅ " if ltyp == v else ""
        await self.safe_edit(update.callback_query,
            "📡 **اختر نوع الروابط:**\n━━━━━━━━━━━━━━━━━━━━━━",
            InlineKeyboardMarkup([
                [btn(f"{mark('telegram')}📲 تيليجرام",  "fetch_type_telegram"),
                 btn(f"{mark('whatsapp')}💬 واتساب",    "fetch_type_whatsapp"),
                 btn(f"{mark('both')}🔄 الاثنين",       "fetch_type_both")],
                [btn("🔙 رجوع", "fetch_select_number")]
            ]))

    async def fetch_set_type(self, update, ctx, ftype):
        ctx.user_data["fetch_type"] = ftype
        await self.safe_ans(update.callback_query, "✅ تم")
        # انتقل للخطوة التالية: اختيار التاريخ
        await self.fetch_date_settings(update, ctx)

    async def fetch_date_settings(self, update, ctx):
        """خطوة 3: اختيار النطاق الزمني."""
        days = ctx.user_data.get("fetch_days", 7)
        def mark(v): return "✅ " if days == v else ""
        await self.safe_edit(update.callback_query,
            "📅 **اختر النطاق الزمني للجلب:**\n━━━━━━━━━━━━━━━━━━━━━━",
            InlineKeyboardMarkup([
                [btn(f"{mark(1)}آخر 24 ساعة",  "fetch_setdate_1"),
                 btn(f"{mark(7)}أسبوع",         "fetch_setdate_7")],
                [btn(f"{mark(30)}شهر",           "fetch_setdate_30"),
                 btn(f"{mark(90)}ثلاثة أشهر",   "fetch_setdate_90")],
                [btn("🔙 رجوع", "fetch_select_type")]
            ]))
    ...
    async def fetch_start(self, update, ctx):
        """بدء الجلب في task خلفي منفصل."""
        uid = update.effective_user.id
        nid = ctx.user_data.get("fetch_nid")
        if not nid:
            await self.safe_ans(update.callback_query, "❌ اختر رقماً أولاً", True)
            return

        if self.fetch.is_running(uid):
            await self.safe_ans(update.callback_query, "⚠️ الجلب يعمل بالفعل!", True)
            return
        ...
        await self.safe_edit(update.callback_query,
            ...
            "🔗 تيليجرام: 0 | 💬 واتساب: 0\n"
            "🔁 مكرر: 0",
            InlineKeyboardMarkup([[btn("⏹️ إيقاف", "fetch_stop")]]))

        async def prog_cb(done, total, tg_c, wa_c, dup_c):
            try:
                bar = helpers.progress_bar(done, total)
                pct = helpers.pct(done, total)
                await msg_ref.edit_text(
                    f"⚡ **جاري البحث الذكي...**\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"{bar}  {pct}\n"
                    f"📂 {done}/{total} محادثة\n\n"
                    f"🔗 تيليجرام: {tg_c} | 💬 واتساب: {wa_c}\n"
                    f"🔁 مكرر: {dup_c}",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([[btn("⏹️ إيقاف", "fetch_stop")]]))
            except Exception:
                pass

        async def break_cb(sec):
            try:
                await msg_ref.edit_text(
                    f"⏸️ **راحة قصيرة للحماية...**\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"⏱️ استئناف بعد {sec} ثانية",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup([[btn("⏹️ إيقاف", "fetch_stop")]]))
            except Exception:
                pass

        async def done_cb(tg, wa, stats):
            ...
            if not tg and not wa:
                msg = err if err else "لم يُعثر على روابط جديدة."
                await msg_ref.edit_text(
                    f"📭 **انتهى الجلب**\n"
                    f"━━━━━━━━━━━━━━━━━━━━━━\n"
                    f"📂 فُحص: {groups_checked} محادثة\n"
        ...
        # شغّل الجلب في task خلفي — البوت لا يتجمد
        asyncio.create_task(
            self.fetch.fetch_links(
                uid, nid, ltyp, days,
                progress_cb=prog_cb,
                break_cb=break_cb,
                done_cb=done_cb))
    ...
    async def fetch_stop(self, update, ctx):
        uid = update.effective_user.id
        self.fetch.stop_fetch(uid)
        await self.safe_ans(update.callback_query, "⏹️ تم إيقاف الجلب", True)
        await self._fetch_confirm_screen(update, ctx)

    async def fetch_clear_memory(self, update, ctx):
        count = self.fetch.clear_memory()
        await self.safe_ans(update.callback_query,
            f"🧹 تم مسح {count} رابط من الذاكرة المؤقتة", True)
        await self._fetch_confirm_screen(update, ctx)

    # أوضاع قديمة — محتفظ بها للتوافق مع router
    ...
    async def fetch_help(self, update, ctx):
        await self.safe_edit(update.callback_query,
            ...
            "البوت يدخل كل محادثة ويبحث مباشرة عن الروابط بدلاً من قراءة كل الرسائل.\n"
            "هذا أسرع وأكثر أماناً من الطريقة التقليدية.\n\n"
            "📡 **أنواع الروابط:**\n"
            "• **تيليجرام** — روابط t.me فقط\n"
            "• **واتساب** — روابط chat.whatsapp.com فقط\n"
            "• **الاثنين** — كلا النوعين معاً\n\n"
            "📅 **النطاق الزمني:**\n"
            "حدد الفترة الزمنية للبحث — كلما كانت أقصر كان الجلب أسرع.\n\n"
            "🛡️ **الأمان:**\n"
            "• راحة 30 ثانية كل 50 محادثة\n"
            "• تأخير عشوائي بين المحادثات\n"
            "• يتجاهل المحادثات المقيدة تلقائياً\n\n"
            "🧠 **الذاكرة المؤقتة:**\n"
            "الروابط المجلوبة تُحفظ لتجنب التكرار. استخدم زر التنظيف عند الانتهاء.",
